...
class Services
{
    ...
    {
        $query = $this->pdo->getPdo()->prepare("SELECT s.id AS id, s.title, s.descricao,  
        ...
        foreach ($RESULT as &$row) {
            if ($row['contratado'] == "nulo") {
                $row['contratado'] = '<button onclick="add_proposal(' . $row["id"] . ')" class="btn btn-primary">Adicionar Proposta</button>';
            }
        }
        return $RESULT;
    }
    ...
    public function addProposal(){
        $loginControl = new LoginController($_SESSION['email'], $_SESSION['password']);

        try{
            $query = $this->pdo->getPdo()->prepare("INSERT INTO easymoney.propostas VALUES
            (NULL, ?, ?, ?)");
            $query->bindValue(1, $_POST['id'], PDO::PARAM_INT);
            $query->bindValue(2, $loginControl->getId(), PDO::PARAM_INT);
            $query->bindValue(3, $_POST['proposal'], PDO::PARAM_STR);
            $query->execute();
            return true;
        }catch(PDOException $e){
            return $e->getMessage();
